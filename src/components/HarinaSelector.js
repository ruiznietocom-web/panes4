const HarinaSelector = ({ onAddPan, cartCount }) => {
  ...
  const toggleHarina = (harina) => {
    setSelectedHarinas(prev => {
      if (prev.find(h => h.id === harina.id)) return prev.filter(h => h.id !== harina.id);
      else if (prev.length < maxHarinas) return [...prev, harina];
      return prev;
    });
  };
  ...
  // Texto dinámico del botón según el número de panes añadidos
  const getButtonText = () => {
    if (cartCount === 0) return 'Elige tus harinas y añade tu pan a la cesta';
    const numeros = ['primer', 'segundo', 'tercer', 'cuarto', 'quinto', 'sexto'];
    return `Elige tu ${numeros[cartCount] || (cartCount + 1) + 'º'} pan`;
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-center mb-4">
        Puedes escoger hasta 5 tipos de harina. Configura y añade todos los panes que quieras.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {harinas.map((harina, index) => (
          <motion.div key={harina.id} className={`relative p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
          ...
          >
            {selectedHarinas.find(h => h.id === harina.id) && (
              <motion.div className="absolute top-2 right-2 bg-amber-500 text-white rounded-full p-1"
